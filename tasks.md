...
- **Phase 2, audience demographics — DONE 2026-09-12.** Migration **0015**
  ...
  The Audience page renders industry, seniority, job function, company
...
- **Migration `0016` (deletion status `failed`) is not yet applied.** Both
  deletion endpoints now record what actually happened rather than what was
  attempted, and `failed` is the value that says so. Until 0016 is applied the
  0003 check constraint rejects it, so the record of a failed erasure is itself
  refused — logged, but lost. Apply it with 0014 and 0015.
